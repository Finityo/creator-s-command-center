...
function simulateDelivery(postId: string, content: string): Response {
  const simId = `sim_instagram_${Date.now()}`;
  console.log(`[SIMULATION] Would post to Instagram:`, {
    postId,
  ...
  return new Response(
    JSON.stringify({
      success: true,
      externalId: simId,
      message: "[SIMULATION] Instagram post simulated successfully",
    }),
...
serve(async (req) => {
  ...
  try {
    ...
    // Real Instagram delivery not yet implemented
    return new Response(
      JSON.stringify({
        success: false,
        error: "Instagram delivery not enabled",
      }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error: any) {
